...
def run_sentinel_check(
    ...
    """Ask the model to echo the sentinel. Returns (found, returned_text)."""
    # probe_user = (
    #     user_text
    #     + "\n\n"
    #     + "TRUNCATION_CHECK: Return ONLY the sentinel string you can see at the very end of this prompt. "
    #       "If you cannot see it, output NOT_FOUND.\n"
    #     + f"{sentinel}"
    # )
    ...
    _pt, _, _, data, _elapsed = post_chat_completion(
        url=url,
        model=model,
        messages=[{"role": "system", "content": system_text}, {"role": "user", "content": probe_user}],
        max_tokens=max_tokens,
        temperature=0.0,
        timeout_s=timeout_s,
    )
    text = extract_text_from_response(data).strip()
    found = (sentinel in text) and ("NOT_FOUND" not in text.upper())
    return found, text
...
def main() -> int:
    ap = argparse.ArgumentParser(description="prompt-token-xray (sentinel): probe truncation risk near context saturation (OpenAI-compatible, LM Studio).")
