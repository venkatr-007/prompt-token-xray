...
def run_sentinel_check(
    ...
    """Ask the model to echo the sentinel (tail visibility check). Returns (found, returned_text).

    Note: we intentionally override the system prompt for this probe so that any JSON/task instructions
    in the main prompts do not interfere with the truncation check.
    """
    probe_system = "TRUNCATION CHECK. Output ONLY the last line of the user's message. No JSON. No extra text."
    # Minimal probe: keep added tokens extremely small.
    probe_user = user_text + "\n\nEcho the last line exactly:\n" + sentinel

    _pt, _, _, data, _elapsed = post_chat_completion(
        url=url,
        model=model,
    ...
    found = (text == sentinel) or (sentinel in text)
...
def main() -> int:
    ...
    ap.add_argument("--sentinel", default="", help="Optional sentinel string for truncation check (disabled if empty).")
    ap.add_argument("--sentinel-threshold-pct", type=float, default=70.0,
                    help="Run sentinel check only when context_pct_used >= this threshold (requires --max-context).")
    ap.add_argument("--sentinel-max-tokens", type=int, default=16,
                    help="Max output tokens for sentinel check response.")
    ap.add_argument("--outdir", default="results", help="Output directory for CSV.")
    ap.add_argument("--out-prefix", default="prompt_token_xray_sentinel", help="CSV filename prefix (without timestamp).")
    args = ap.parse_args()
    ...
    with open(out_csv, "w", newline="", encoding="utf-8") as f:
        ...
        for m in multipliers:
            ...
            try:
                ...
                # If the server is too slow (large context / heavy prompt), record a row and stop gracefully.
                ...
                try:
                    ...
                    if "ReadTimeout" in type(e2).__name__ or "timed out" in str(e2).lower():
                        row.update({
                            "system_prompt_tokens": None,
                            "user_prompt_tokens": None,
                            "combined_prompt_tokens": None,
            ...
            # Optional: sentinel-based truncation check (runs only near saturation).
            if args.sentinel and args.max_context and row.get("combined_prompt_tokens") is not None:
                try:
                    pct_val = float(row["context_pct_used"]) if row.get("context_pct_used") else 0.0
                except Exception:
                    pct_val = 0.0

                if pct_val >= float(args.sentinel_threshold_pct):
                    try:
                        found, returned = run_sentinel_check(
                            url=args.url,
                            model=args.model,
                            system_text=system_text,
                            user_text=user_text,
                        ...
                        row["sentinel"] = args.sentinel
                        row["sentinel_returned"] = returned[:200]
                        row["truncation_suspected"] = "true" if not found else "false"
                        if not found:
                            notes.append("SENTINEL_MISSING: possible truncation or model did not follow instruction.")
                    except Exception as se:
                        if is_context_overflow_error(se):
                            row["sentinel"] = args.sentinel
                            row["sentinel_returned"] = ""
                            row["truncation_suspected"] = "true"
                            notes.append("SENTINEL_CHECK_OVERFLOW: probe request overflowed (likely near/over context).")
                        else:
                            notes.append(f"SENTINEL_CHECK_ERROR: {type(se).__name__}: {se}")
            ...
            print(
                ...
                + (f" truncation={row['truncation_suspected']}" if row.get("truncation_suspected") else "")
            )
